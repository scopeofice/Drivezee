...
export default function PostalCodeUpdate() {
  ...
  useEffect(() => {
    ...
    fetchAddressAndPostalCode(); // Fetch address and postal code when component mounts
  }, []);

  const fetchAddressAndPostalCode = async () => {
    ...
    if (bookingId) {
      try {
        ...
        if (!response.data.error) {
          const { address, postal_code } = response.data.data;
          setPickupAddress(address || ""); // Set address if available, otherwise set to empty string
          if (postal_code) {
            const postalCodeArray = postal_code.split("");
            setInputCodes(postalCodeArray);
            setShowTextBox(true);
          }
        } else {
        ...
        console.error("Error fetching address and postal code:", error);
      }
    }
  };
  ...
  const handleNextButtonClick = () => {
    if (!showTextBox) {
      ...
      localStorage.setItem("zipcodeUpdate", encryptedPostalCode);
      localStorage.setItem("pickupAddressUpdate", encryptedPickupAddress);
      console.log(
        CryptoJS.AES.decrypt(encryptedPostalCode, secretKey).toString(
          CryptoJS.enc.Utf8
        )
      );
      navigate("/slotselectionupdate");
    }
  };
